...
function __api_proxy() {
	...
	for f in $STELLA_API_COMMON_PUBLIC $STELLA_API_BUILD_PUBLIC $STELLA_API_APP_PUBLIC $STELLA_API_API_PUBLIC $STELLA_API_FEATURE_PUBLIC $STELLA_API_NETWORK_PUBLIC $STELLA_API_PLATFORM_PUBLIC; do
		if [ "$f" == "$FUNC_NAME" ]; then
