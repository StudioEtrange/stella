...
function __get_box() {
	...
	__get_resource "$_distrib_id" "$VAGRANT_BOX_URI" "$VAGRANT_BOX_URI_PROTOCOL"
	__import_box_into_vagrant $VAGRANT_BOX_NAME "$STELLA_APP_CACHE_DIR/$VAGRANT_BOX_FILENAME"
}
...
function __create_box() {
	...
	if [ ! -f "$STELLA_APP_CACHE_DIR/$VAGRANT_BOX_FILENAME" ]; then
		...
		if [ ! "$PACKER_TEMPLATE_URI_PROTOCOL" == "_INTERNAL_" ]; then
			__get_resource "$DISTRIB" "$PACKER_TEMPLATE_URI" "$PACKER_TEMPLATE_URI_PROTOCOL" "$VIRTUAL_TEMPLATE_ROOT/$DISTRIB" "$STELLA_APP_CACHE_DIR"
