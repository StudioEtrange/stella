...
:install_cmake
	...
	if not exist %STELLA_APP_TOOL_ROOT%\cmake mkdir %STELLA_APP_TOOL_ROOT%\cmake
	if "%_VER%"=="" (
...
:install_cmake_2_8_12
	...
	set "INSTALL_DIR=%STELLA_APP_TOOL_ROOT%\cmake\%VERSION%-win32-x86"
...
:feature_cmake_2_8_12
	...
	set FEATURE_PATH=
	set FEATURE_VER=
	if exist "%STELLA_APP_TOOL_ROOT%\cmake\2_8_12-win32-x86\bin\cmake.exe" (
		set "TEST_FEATURE=%STELLA_APP_TOOL_ROOT%\cmake\2_8_12-win32-x86"
	)
