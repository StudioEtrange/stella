...
:install_openssh
	...
	if not exist %TOOL_ROOT%\openssh mkdir %TOOL_ROOT%\openssh
	if "%_VER%"=="" (
...
:install_openssh_6_6
	...
	set INSTALL_DIR="%TOOL_ROOT%\openssh\6_6"
...
:feature_openssh_6_6
	...
	set FEATURE_PATH=
	set FEATURE_VER=
	if exist "%TOOL_ROOT%\openssh\6_6\bin\ssh.exe" (
		set "TEST_FEATURE=%TOOL_ROOT%\openssh\6_6"
	)
	if not "!TEST_FEATURE!"=="0" (
